...
\begin{figure}[t]
    ...
    \begin{subfigure}[b]{0.32\textwidth}
        \centering
        \includegraphics[width=\textwidth]{sensitivity_cin.eps}
    ...
    \begin{subfigure}[b]{0.32\textwidth}
        \centering
        \includegraphics[width=\textwidth]{sensitivity_cout.eps}
    ...
    \caption{
    Sensitivity analysis of TensorRT convolution on NVIDIA Blackwell RTX PRO 3000.
...
We conducted sensitivity studies using TensorRT by varying the number of output channels ($C_\mathrm{out}$) from 2 to 32 in steps of 2 while keeping the number of input channels fixed at $C_\mathrm{in}=64$, and observed that configurations where $C_\mathrm{out}$ is a multiple of 4 or 8 consistently achieve lower execution time than other cases. Likewise, when varying the number of input channels ($C_\mathrm{in}$) from 2 to 32 in steps of 2 while keeping the number of output channels fixed at $C_\mathrm{out}=8$, we observe similiar pattern, which we attribute to TensorRT’s kernel selection favoring convolution filter dimensions that are multiples of 4 or 8. These experiments were conducted on NVIDIA Blackwell GPUs~\cite{nvidia_blackwell}. Overall, these results highlight the impact of using TensorRT together with architecture-specific alignment and kernel selection constraints on convolution performance.
...
perspective of space folding, and Section~\ref{sec:correctness}  shows
the mathematical correctness of the transformation.
...
requirement for SIMD or block formats ~\cite{onednn2025}) 
...
Fourth, we like to extend this to FP8 and FP4 datatypes and other data formats
(e.g. kCHW4 or kCHW16) in TensorRT and oneDNN.
