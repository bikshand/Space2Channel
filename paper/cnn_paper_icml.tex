...
\item Eemonstrate generalization to GEMM via $1\times1$ convolutions.
...
{Input Feature Map\\
$\mathbf{x} \in \mathbb{R}^{ H \times W \times Cin}$};

\node[box, fill=filtergreen] (filter1) at (0,0)
{Filter Weights\\
$\mathbf{W} \in \mathbb{R}^{K \times Cin}$};
...
{Folded Input\\
	$\tilde{\mathbf{x}} \in \mathbb{R}^{H \times (W \div F) \times (Cin \dot F))}$};

\node[box, fill=filtergreen] (filter2) at (10,0)
{Block-Diagonal Filter\\
$\tilde{\mathbf{W}} \in \mathbb{R}^{K \times (Cin \cdot F)}$};
...
Zero-padding simply adds empty channels. The convolution computation over these extra channels does nothing useful—thesechannels carry no information. Even though the number of channels is aligned, Tensor Cores may still process these zero channels, wasting compute cycles on irrelevant data.
zero-padding does not create a predictable pattern that frameworks can exploit. In contrast, block-diagonal filters introduce structured sparsity, where off-diagonal blocks are zero but the diagonal blocks contain actual filter weights. Frameworks can leverage this for efficient computation (skipping zeros).
Finally, Block-diagonal filters naturally generalize to higher dimensions and grouped structures.
...
requirement for SIMD or cache usage) remains an open and important problem.
